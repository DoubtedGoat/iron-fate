...
paths.each do |file|
  ...
  db.execute "insert into characters (name, shortname, base_class, heart_class, alt_heart_class, prepromote) values (?,?,?,?,?,?)",
    params['name'], shortname, params['base_class'], params['heart_class'], params['alt_heart_class'], params['prepromote']
  ...
  db.execute "insert into characters_to_skills (character, skill) values (?, ?)",
               shortname, skill
...
def sets(type, shortname, params, db)
  if not params[type].nil? then
    params[type].each do |friend, friendclass|
      puts friend
      puts friendclass
      db.execute "insert into #{type} (character, support_character, class) values (?,?,?)",
                   shortname, friend, friendclass
    end
  end
end
