...
paths.each do |file|
  ...
  params['skills'].each_with_index do |skill, index|
    ...
    db.execute "insert into classes_to_skills (level, class, skill) values (?,?,?)",
                 level, shortname, skill
  end
  ...
  params['weapons'].each_with_index do |weapon, index|
    ...
    db.execute "insert into classes_to_weapons (class, weapon) values (?,?)",
                 shortname, weapon
  end
end
...
paths.each do |file|
  ...
  params['promotions'].each do |promotion|
    db.execute "insert into classes_to_promotions (class, promotion) values (?,?)",
                 shortname, promotion
  end
end
